...
def generate_gemini_from_prompt(prompt: str, fallback_text: str, api_key: str | None = None) -> str:
    ...
    if not key:
        return f"Gemini key missing. Showing local fallback explanation.\n\n{fallback_text}"
    ...
    try:
        ...
        if "quota" in last_error_text.lower() or last_error_name == "ResourceExhausted":
            return f"Gemini quota exceeded on all configured models. Showing local fallback explanation.\n\n{fallback_text}"

        return f"Gemini unavailable ({last_error_name}) on all configured models. Showing local fallback explanation.\n\n{fallback_text}"
    except Exception as e:
        error_text = str(e)
        error_name = type(e).__name__

        if "quota" in error_text.lower() or error_name == "ResourceExhausted":
            return f"Gemini quota exceeded. Showing local fallback explanation.\n\n{fallback_text}"

        return f"Gemini unavailable ({error_name}). Showing local fallback explanation.\n\n{fallback_text}"
